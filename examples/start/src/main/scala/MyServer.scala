...
object myServer extends zio.ZIOAppDefault {
  ...
  def run = {
    ...
    // Post filter, response header modifiations
    ...
    //various app routes
    val app_routes: HttpRoutes[Any] = HttpRoutes.of { (req: Request) =>
      {
        req match {
          //Multipart Save All example. You can use Multipart stream as well and have custom procssing.
          ...
          //Methods and parameters, print headers  
          ...
          //File GET examples. chunked and non chunked
          //contentType() and transferEncoding("chunked")
          case req @ GET -> "files" /: "chunked" /: remainig_path =>
            for {
    ...
    val myHttpRouter = new HttpRouter(
      /* normal app routes */
      List(app_routes, app_route_JSON, app_route_pre_post_filters, ws_stream)
    )
