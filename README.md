...
* It has been decided that we try to build a cache based on home grown fixed SkipList. Initial tests are very positive. 
 ...
 So, why not use to more general approach for 100% LockFree HashMap implementation - https://github.com/ollls/mycas. ( sbt test ). ( yes - remove is costly, but it's a cache ).
